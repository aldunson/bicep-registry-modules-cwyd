...
// Feature flags
param azureSearchUseIntegratedVectorization = bool(readEnvironmentVariable(
...
param azureOpenAIEmbeddingModelName = readEnvironmentVariable(
  'AZURE_OPENAI_EMBEDDING_MODEL_NAME',
  'text-embedding-ada-002'
)
param azureOpenAIEmbeddingModelVersion = readEnvironmentVariable('AZURE_OPENAI_EMBEDDING_MODEL_VERSION', '2')
param azureOpenAIMaxToks = readEnvironmentVariable('AZURE_OPENAI_MAX_TOKENS', '1000')
...
param azureOpenAISystemMessage = readEnvironmentVariable(
...
param computerVisionVectorizeImageApiVersion = readEnvironmentVariable(
  'AZURE_COMPUTER_VISION_VECTORIZE_IMAGE_API_VERSION',
  '2024-02-01'
)
param computerVisionVectorizeImageModelVersion = readEnvironmentVariable(
  'AZURE_COMPUTER_VISION_VECTORIZE_IMAGE_MODEL_VERSION',
  '2023-04-15'
)
...
param resourceName = toLower(uniqueString(subscriptionId, environmentName, location))
...
param azureAISearchName = searchServiceName == '' ? 'search-${resourceName}' : searchServiceName

param azureSearchIndex = readEnvironmentVariable('AZURE_SEARCH_INDEX', 'index-${resourceName}')
param azureOpenAIResourceName = readEnvironmentVariable('AZURE_OPENAI_RESOURCE', 'openai-${resourceName}')
param storageAccountName = readEnvironmentVariable('AZURE_BLOB_ACCOUNT_NAME', 'str${resourceName}')
